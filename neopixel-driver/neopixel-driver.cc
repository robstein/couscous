...
//
// This simulation scales it self a bit depending on MATRIX_HEIGHT; it should
...
uint16_t fire2012WithPalette() {
  ...
  for (uint8_t x = 0; x < MATRIX_WIDTH; x++) {
    ...
    for (int i = 0; i < MATRIX_HEIGHT; i++) {
      heat[x][i] =
          qsub8(heat[x][i], random8(0, ((COOLING * 10) / MATRIX_HEIGHT) + 2));
    }
